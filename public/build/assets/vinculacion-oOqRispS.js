const f=document.querySelector('meta[name="csrf-token"]').getAttribute("content");document.addEventListener("DOMContentLoaded",function(){let l=[],d=[],p=new Map;const v=document.getElementById("mecanicos-list"),h=document.getElementById("supervisores-list"),s=document.getElementById("vinculacion-tbody");function S(r){var t;return((t=p.get(r))==null?void 0:t.cvetra)||""}function g(r){const t=S(r);return t?`http://128.150.102.45:8000/Intimark/${t}.jpg`:"/default-avatar.jpg"}function $(){fetch("/mecanicos").then(r=>r.json()).then(r=>{l=r,p.clear(),l.forEach(t=>{p.set(t.nombre,t)}),v.innerHTML=l.map(t=>`
                ...
                `).join(""),b()})}function k(){fetch("/supervisores").then(r=>r.json()).then(r=>{d=r,h.innerHTML=d.map(t=>`
            ...
            `,s.insertBefore(r,s.firstChild)),r}function b(){if(l.length&&d.length){const r={animation:150,forceFallback:!1,delayOnTouchOnly:!0,delay:100,touchStartThreshold:5,fallbackTolerance:3,ghostClass:"bg-blue-100"};new Sortable(v,{...r,group:{name:"shared",pull:"clone",put:!1},sort:!1,onClone:t=>{requestAnimationFrame(()=>{t.item.style.transform="translate3d(0, 0, 0)"})}}),new Sortable(h,{...r,group:{name:"shared",pull:"clone",put:!1},sort:!1}),new Sortable(s,{...r,group:{name:"shared",pull:!1,put:!0},onAdd:function(t){requestAnimationFrame(()=>{const o=t.item,e=parseInt(o.getAttribute("data-index"));let a=C();if(a!==s.firstChild&&s.insertBefore(a,s.firstChild),o.classList.contains("draggable-mecanico")){const n=l[e];n&&w(a,n)}else if(o.classList.contains("draggable-supervisor")){const n=d[e];n&&B(a,n)}o.remove()})}})}}function w(r,t){const o=r.querySelector('[name="mecanico"]'),e=o.querySelector("img");e.src=g(t.nombre),e.alt=t.cvetra,o.querySelector(".mecanico-nombre").textContent=t.nombre,r.setAttribute("data-cvetra",t.cvetra)}function B(r,t){const o=r.querySelector('[name="supervisor-modulo"]');o.innerHTML=`Mod: ${t.Modulo}<br>Sup: ${t.Nombre}`,r.setAttribute("data-supervisor",t.Nombre),r.setAttribute("data-modulo",t.Modulo)}window.validateTimeRange=function(r){const t=r.closest("div"),o=t.querySelector("select:first-child"),e=t.querySelector("select:last-child"),a=r.closest("td").getAttribute("name").replace("-"," "),n=o.value,i=e.value;return n&&i&&n>=i?(Swal.fire({title:"Error en horario",text:`La hora de fin debe ser mayor a la hora de inicio en ${a}`,icon:"warning",confirmButtonColor:"#d33"}).then(()=>{r.value=""}),!1):!0};function c(r="",t=""){const o=[];for(let e=8;e<=17;e++)["00","10","15","20","25","30","35","40","45","50"].forEach(a=>{(e!==17||e===17&&a<="50")&&o.push(`${String(e).padStart(2,"0")}:${a}`)});return`<div class="flex items-center gap-1">
        ...
        </div>`}$(),k(),window.deleteVinculacion=function(r,t){Swal.fire({title:"¿Está seguro?",text:"Esta acción no se puede deshacer",icon:"warning",showCancelButton:!0,confirmButtonColor:"#d33",cancelButtonColor:"#3085d6",confirmButtonText:"Sí, eliminar",cancelButtonText:"Cancelar"}).then(o=>{o.isConfirmed&&fetch(`/vinculaciones/${t}`,{method:"DELETE",headers:{"Content-Type":"application/json","X-CSRF-TOKEN":f}}).then(e=>e.json()).then(e=>{if(e.success)Swal.fire("Eliminado","El registro ha sido eliminado.","success"),r.closest("tr").remove();else throw new Error(e.message)}).catch(e=>{console.error("Error:",e),Swal.fire("Error","No se pudo eliminar el registro","error")})})};function y(){s.innerHTML=`
            ...
            <tr class="animate-pulse bg-white dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                `})}window.deleteGroupedVinculaciones=function(r,t){Swal.fire({title:"¿Está seguro?",text:"Se eliminarán todas las vinculaciones de este grupo",icon:"warning",showCancelButton:!0,confirmButtonColor:"#d33",cancelButtonColor:"#3085d6",confirmButtonText:"Sí, eliminar",cancelButtonText:"Cancelar"}).then(o=>{if(o.isConfirmed){const e=t.split(",");Promise.all(e.map(a=>fetch(`/vinculaciones/${a}`,{method:"DELETE",headers:{"Content-Type":"application/json","X-CSRF-TOKEN":f}}).then(n=>n.json()))).then(()=>{Swal.fire("Eliminado","Los registros han sido eliminados.","success"),r.closest("tr").remove()}).catch(a=>{console.error("Error:",a),Swal.fire("Error","No se pudieron eliminar los registros","error")})}})},document.getElementById("guardar-vinculacion").addEventListener("click",function(){const r=Array.from(s.children);let t=!0;for(const e of r){const a=["comida","break-lj","break-v"];for(const n of a){const i=e.querySelector(`[name="${n}"]`),u=i.querySelector("select:first-child"),m=i.querySelector("select:last-child");if(!u.value||!m.value){Swal.fire({title:"Campos incompletos",text:`Por favor complete los horarios de ${n.replace("-"," ")}`,icon:"warning",confirmButtonColor:"#3085d6"}),t=!1;return}if(u.value>=m.value){Swal.fire({title:"Error en horarios",text:`La hora de fin debe ser mayor a la hora de inicio en ${n.replace("-"," ")}`,icon:"warning",confirmButtonColor:"#d33"}),t=!1;return}}}if(!t)return;const o=Array.from(s.children).flatMap(e=>{var x;const a=((x=e.getAttribute("data-ids"))==null?void 0:x.split(","))||[],n=e.querySelectorAll('[name="mecanico"] .mecanico-nombre'),i=e.getAttribute("data-supervisor"),u=e.getAttribute("data-modulo"),m={Hora_Comida_Inicio:e.querySelector('[name="comida"] select:first-child').value,Hora_Comida_Fin:e.querySelector('[name="comida"] select:last-child').value,Break_Lun_Jue_Inicio:e.querySelector('[name="break-lj"] select:first-child').value,Break_Lun_Jue_Fin:e.querySelector('[name="break-lj"] select:last-child').value,Break_Viernes_Inicio:e.querySelector('[name="break-v"] select:first-child').value,Break_Viernes_Fin:e.querySelector('[name="break-v"] select:last-child').value};return Array.from(n).map((_,E)=>({id:a[E]||null,Supervisor:i,Modulo:u,Mecanico:_.textContent.trim(),...m}))});Swal.fire({title:"¿Desea guardar la información?",icon:"question",showCancelButton:!0,confirmButtonColor:"#3085d6",cancelButtonColor:"#d33",confirmButtonText:"Guardar",cancelButtonText:"Cancelar"}).then(e=>{e.isConfirmed&&fetch("/vinculaciones",{method:"POST",headers:{"Content-Type":"application/json","X-CSRF-TOKEN":f},body:JSON.stringify({vinculaciones:o})}).then(a=>a.json()).then(a=>{if(a.success)Swal.fire({title:"Éxito",text:a.message||"Vinculaciones guardadas correctamente",icon:"success",confirmButtonColor:"#3085d6"}).then(()=>{y()});else throw new Error(a.message||"Error al guardar")}).catch(a=>{console.error("Error al guardar vinculaciones:",a),Swal.fire({title:"Error",text:"Error al guardar las vinculaciones",icon:"error",confirmButtonColor:"#d33"})})})}),y()});
