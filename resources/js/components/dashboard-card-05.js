    ...
    fetch('/json-data-feed?datatype=5&limit=' + range)
        ...
        .then(result => {
            ...
            document.addEventListener('darkMode', (e) => {
                const { mode } = e.detail;
                const colorScheme = mode === 'on' ? 'dark' : 'light';
                chart.options.scales.x.ticks.color = textColor[colorScheme];
                chart.options.scales.y.ticks.color = textColor[colorScheme];
                chart.options.scales.y.grid.color = gridColor[colorScheme];
                chart.options.plugins.tooltip.titleColor = tooltipTitleColor[colorScheme];
                chart.options.plugins.tooltip.bodyColor = tooltipBodyColor[colorScheme];
                chart.options.plugins.tooltip.backgroundColor = tooltipBgColor[colorScheme];
                chart.options.plugins.tooltip.borderColor = tooltipBorderColor[colorScheme];
                chart.update('none');
            });
        });
};
