...
class FollowAtentionV2Controller extends Controller
{
    ...
    {
        return view('followOT.monitor');
    }

    public function monitorData(ReporteIndicadorRojoService $reporteIndicadorRojoService)
    {
        try {
            $datosMonitor = $reporteIndicadorRojoService->obtenerDatosMonitor();

            return response()->json([
                'success' => true,
                'actualizado_en' => $datosMonitor['actualizado_en'],
                'tickets_en_atencion' => $datosMonitor['tickets_en_atencion'],
                'tickets_sin_iniciar' => $datosMonitor['tickets_sin_iniciar'],
            ]);
        } catch (\Exception $e) {
            Log::error('Error al obtener datos del monitor de OT: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
            ]);

            return response()->json(['success' => false, 'message' => 'No se pudieron cargar los datos del monitor'], 500);
        }
    }
