...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    topsData = loadFromLocalStorage(TOPS_STORAGE_KEY, TOPS_STORAGE_TTL);
    ...
    if (!topsData) {
        try {
            ...
            saveToLocalStorage(TOPS_STORAGE_KEY, topsData);
        } catch (e) {
    ...
    `;
    const maquinas = (topsData.maquinas || []).map(m => ({
        label: m.Maquina,
        total: m.total
    }));
    ...
    const problemas = (topsData.problemas || []).map(p => ({
    ...
    const modulos = (topsData.modulos || []).map(m => ({
        label: m.Modulo,
        total: m.total
    }));
